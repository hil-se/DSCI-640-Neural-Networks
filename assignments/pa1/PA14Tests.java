...
public class PA14Tests {
    ...
    public static void testNormalize() {
        ...
        double[] actualInput = new double[]{-0.8976738791967672, 1.0286112808972334, -1.3367940202882518, -1.3085928194379581};

        if (BasicTests.vectorsCloseEnough(means, actualMeans)) {
            Log.info("passed mean calculation from iris data set!");
        } else {
            Log.error("failed calculation from iris data set!");
        }

        if (BasicTests.vectorsCloseEnough(stdDevs, actualStdDevs)) {
            Log.info("passed standard deviation calculation from iris data set!");
        } else {
            Log.error("failed standard deviation calculation from iris data set!");
        }

        if (BasicTests.vectorsCloseEnough(irisData.getInstance(0).inputs, actualInput)) {
            Log.info("passed normalization test from iris data set!");
        } else {
            Log.error("failed normalization test from iris data set!");
        }
     }
