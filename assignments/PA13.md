...
In NeuralNetwork.forwardPass(Instance instance) you will need to implement the L1_NORM and L2_NORM loss functions. Some wrapper code and TODOs show where you should do this. Make sure you update the gradients for the postActivationValues for the output node(s) after the loss function, and set their delta values to the appropriate gradient (the gradients are from the lecture notes).
...
java PA13GradientDescent.java <data set=""> <network type=""> <gradient descent="" type=""> <loss function=""> <epochs> <bias> <learning rate="">
...
Programming Assignment 1 is 28% of your final grade, and PA1-3 is 25% of that. It will be traded out of 7 points as follows:
...
(1 point * 3 = 3 points) implementing stochastic, minibatch and batch gradient descent epochs correctly.

(1 point) Run the PA13GradientDescent code as follows and save a screenshot of your output as [screenshot.png](). It should look like below:
```
java PA13GradientDescent xor large minibatch l1_norm 20 0.1 1
```
Expected output:
<img width="827" height="839" alt="image" src="https://github.com/user-attachments/assets/485a8ff1-b6d9-4197-b722-d0ead802e427" />

NOTE: For each of the datasets, even if you've implemented everything correctly, you may notice that your error will start around 2 and get stuck around 1.0. Sometimes however it will get down to 0. This is because these search landscapes have some flat areas which the network can get stuck on around 1.0. So this is not a bug with your implementation. When we add momentum in PA1-4 we should be able to break free from these regions better.
